    ...
    <div style="max-height: 90vh; overflow-y: auto">
      ...
      <ul v-else-if="audits.length > 0" class="max-h-xs list-disc overflow-y-auto ps-4">
        <li v-for="item in audits" :key="item.id">
          <span
            class="me-2 inline-block font-bold"
            :class="classFormatter ? classFormatter(item.new_value) : ''"
          >
            {{ formatter ? formatter(item.new_value) : item.new_value }}
          </span>
          <span
            v-tooltip.sm="item.user?.name ? t('By') + ': ' + item.user?.name : undefined"
            class="dir-ltr inline-block text-xs"
          >
