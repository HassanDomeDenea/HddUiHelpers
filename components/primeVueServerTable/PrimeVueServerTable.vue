...
const recordsService = {
  async getRecords(payload: RequestDataPayloadType) {
    if (props.customService?.getRecords) return await props.customService.getRecords(payload, { url: props.url, routeName: props.routeName });
  ...
  async deleteRecord(id: number) {
    if (props.customService?.deleteRecord) return await props.customService.deleteRecord(id, { url: props.url, routeName: props.routeName });
...
const firstInPage = computed(() => {
  if (rowsPerPage.value === -1) return Math.min(1, totalRecords.value);
  return Math.min(first.value + 1, totalRecords.value);
});

const lastInPage = computed(() => {
  if (rowsPerPage.value === -1) return totalRecords.value;
...
const contextMenuModel = computed<MenuItem[]>(() => {
  ...
  if (props.extraContextMenuOptions?.length > 0) {
    list.push(
      ...cloneDeep(props.extraContextMenuOptions).map((e) => {
        if (e.command) {
...
async function getData() {
  ...
  if (props.url || props.routeName) {
    await recordsService
      .getRecords(requestDataPayload.value)
      .then((res) => {
        if (res.success) {
...
function onSelectAllChange(event: DataTableSelectAllChangeEvent) {
  ...
  if (selectAll.value) selectedRecords.value = records.value;
...
function deleteRecords(item: T | T[]) {
  ...
  confirm.require({
    ...
    accept: async () => {
      ...
      try {
        if (props.url) {
          if (Array.isArray(item)) await recordsService.deleteRecordsMulti(ids);
          else await recordsService.deleteRecord(item.id);
        } else {
          // TODO: Local Table
        }
        toast.add({
...
const dialogFormWrapperRecordsListValue = ref([]);
...
function showEditDialog(item: T | T[]) {
  if (Array.isArray(item)) {
    dialogFormWrapperRef.value?.editMulti(item);
  } else {
...
<template>
  <div class="p-server-datatable-container max-w-full px-4">
          ...
          <table class="printable-table mx-auto mt-3">
            <thead>
              <tr>
                ...
                <template v-for="column in columns">
                  <th v-if="column.printable !== false && checkColumnIsVisible(column)" :key="column.name">
            ...
            <tbody>
              <tr v-for="(row, rowIndex) in recordsToPrint || records" :key="row[dataKeyId]">
                ...
                <template v-for="column in columns">
                  <td v-if="column.printable !== false && checkColumnIsVisible(column)" :key="column.name">
                    <slot :name="`${column.name.replace('.', '_')}ColumnBody`" :row="row as T" :data="lodashGet(row, column.name)">
                      <div :class="column.bodyClassFunction ? column.bodyClassFunction(lodashGet(row, column.name)) : ''">
    ...
    <DialogFormWrapper
      ref="dialogFormWrapperRef"
      v-bind="dialogFormWrapperOptions"
    ...
    <DataTable
      ...
      :row-class="
        (row) => [isActiveRow && isActiveRow(row) ? 'active-row' : '', rowClass ? rowClass(row) : '', { 'cursor-pointer': hasRowClickEventListener }]
      "
      :row-hover="rowHover"
    ...
    >
      <template #header>
        ...
        <div class="flex w-full items-end">
          <div v-if="hasGlobalFilter" class="justify-content-between flex flex-1 gap-1">
            <IconField>
          ...
          <div class="flex flex-1 justify-end gap-1">
            ...
            <Button
              ...
              v-tooltip="{ class: 'warn', value: selectedRecords.length > 1 && !multiEditable ? t('You must select one item only') : null }"
              size="small"
              :disabled="selectedRecords.length < 1 || (selectedRecords.length !== 1 && !multiEditable)"
              severity="success"
              icon="i-mdi-edit"
              :label="t('Edit')"
              ...
              <Popover ref="visibleColumnsPopoverRef">
                ...
                >
                  <Checkbox
                    ...
                    :input-id="'ColumnVisibilityCheckbox_' + col.name"
                    :value="col.name"
                    @change="saveVisibleColumnsState"
                  />
                  <label :for="'ColumnVisibilityCheckbox_' + col.name" class="px-1">{{ col.header || col.name }}</label>
                </div>
              </Popover>
              <Button
      ...
      <template v-for="column in columns" :key="column.name">
        <Column
          ...
          :field="column.name"
          :header="column.header || (autoI18nColumnsHeader ? t(ucFirst(column.name)) : column.name)"
          :show-filter-match-modes="
            isBoolean(column.showFilterMatchModes) ? column.showFilterMatchModes : column.type === 'select' ? false : enableFilterMatchModes
          "
          :show-add-button="isBoolean(column.showFilterAddButton) ? column.showFilterAddButton : enableFilterAddButton"
          :show-apply-button="
            isBoolean(column.showFilterApplyButton) ? column.showFilterApplyButton : column.type === 'select' ? false : enableFilterApplyButton
          "
          :show-clear-button="
            isBoolean(column.showFilterClearButton) ? column.showFilterClearButton : column.type === 'select' ? false : enableFilterClearButton
          "
          ...
          v-bind="column.props || {}"
        >
          <template v-if="isBoolean(column.filterable) ? column.filterable : enableColumnFilters" #filter="{ filterModel, filterCallback }">
            <slot :name="`${column.name.replace('.', '_')}ColumnFilter`" :slot-props="{ filterModel, filterCallback }">
              ...
              <template v-if="column.type === 'numeric'">
                <template v-if="['between', 'notBetween'].indexOf(filterModel.matchMode) > -1">
                  <div v-for="i in [0, 1]" :key="i">
                    ...
                    <InputText
                      ...
                      @input="
                        filterModel.value = [
                          i === 0 ? +$event.target.value : (filterModel.value || [])[0],
                          i === 1 ? +$event.target.value : (filterModel.value || [])[1],
                        ]
                      "
                      @keydown.enter="filterCallback()"
                    />
              ...
              <template v-if="column.type === 'boolean'">
                <Checkbox v-model="filterModel.value" class="mr-1" :false-value="0" :true-value="1" binary @change="filterCallback()" />
      ...
      <Column v-if="toolsColumn" body-class="p-tools-cell" :align-frozen="t('dir')" :frozen="frozenToolsColumn" v-bind="toolsColumnProps">
        <template #header>
          <i class="i-mdi-tools mx-auto" />
        </template>
        <template #body="row">
          <div class="flex flex-wrap items-center gap-1">
            <Button v-if="openable" v-tooltip="t('Open')" severity="info" size="small" class="rounded-md p-2" @click="emits('rowOpen', row.data)">
              <i class="i-mdi-open-in-app" />
            </Button>
            <Button v-if="editable" v-tooltip="t('Edit')" severity="success" size="small" class="rounded-md p-2" @click="editRecord(row.data)">
              <i class="i-mdi-edit" />
            </Button>
            <Button v-if="deletable" v-tooltip="t('Remove')" severity="danger" size="small" class="rounded-md p-2" @click="deleteRecords(row.data)">
              <i class="i-mdi-trash" />
            </Button>
