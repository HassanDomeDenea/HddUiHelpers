...
const recordsService = {
  async getRecords(payload: RequestDataPayloadType) {
    if (props.customService?.getRecords)
      return await props.customService.getRecords(payload, {
        url: props.url,
        routeName: props.routeName,
      });
    else if (dataTableRecordsService?.getRecords)
      return await dataTableRecordsService.getRecords(payload, {
        url: props.url,
        routeName: props.routeName,
      });
    let path: string;
    ...
    return (
      await axiosInstance.get(path, {
        params: payload,
        signal: getRecordsAbortController.value.signal,
      })
    ).data;
  },
  async deleteRecord(id: number) {
    if (props.customService?.deleteRecord)
      return await props.customService.deleteRecord(id, {
        url: props.url,
        routeName: props.routeName,
      });
    else if (dataTableRecordsService?.deleteRecord)
      return await dataTableRecordsService.deleteRecord(id, {
        url: props.url,
        routeName: props.routeName,
      });
  ...
  async deleteRecordsMulti(ids: number[]) {
    if (props.customService?.deleteRecordsMulti)
      return await props.customService.deleteRecordsMulti(ids, {
        url: props.url,
        routeName: props.routeName,
      });
    else if (dataTableRecordsService?.deleteRecordsMulti)
      return await dataTableRecordsService.deleteRecordsMulti(ids, {
        url: props.url,
        routeName: props.routeName,
      });
...
<template>
  <div class="p-server-datatable-container max-w-full px-4">
            ...
            <tbody>
              <tr v-for="(row, rowIndex) in recordsToPrint || records" :key="row[dataKeyId]">
                ...
                <template v-for="column in columns">
                  ...
                  >
                    <slot
                      :name="`${column.name.replace('.', '_')}ColumnBody`"
                      :row="row as T"
                      :data="lodashGet(row, column.name)"
                    >
                      <div
                        :class="
                          column.bodyClassFunction
                            ? column.bodyClassFunction(lodashGet(row, column.name))
                            : ''
                        "
                      >
    ...
    <ContextMenu
      ...
      @hide="contextMenuSelectedProduct = undefined"
    />
    <DataTable
      ref="dtRef"
    ...
    >
        ...
        <div class="flex w-full items-end">
          ...
          <div class="flex flex-1 justify-end gap-1">
            ...
            <Button
              ...
              severity="secondary"
              :icon="
                selectAll ? 'i-mdi:square-rounded-outline' : 'i-fluent:select-all-on-24-regular'
              "
            ...
            <Button
              ...
              size="small"
              :disabled="
                selectedRecords.length < 1 || (selectedRecords.length !== 1 && !multiEditable)
              "
              severity="success"
              icon="i-mdi-edit"
              :label="t('Edit')"
      ...
      <template v-for="column in columns" :key="column.name">
        <Column
          ...
          :hidden="
            column.visibilityControl !== false
              ? visibleColumns.indexOf(column.name) < 0
              : isBoolean(column.visible)
                ? !column.visible
                : false
          "
          ...
          :show-filter-match-modes="
            isBoolean(column.showFilterMatchModes)
          ...
          "
          :show-add-button="
            isBoolean(column.showFilterAddButton)
              ? column.showFilterAddButton
              : enableFilterAddButton
          "
          :show-apply-button="
            isBoolean(column.showFilterApplyButton)
              ? column.showFilterApplyButton
              : column.type === 'select'
                ? false
                : enableFilterApplyButton
          "
          :show-clear-button="
            isBoolean(column.showFilterClearButton)
              ? column.showFilterClearButton
              : column.type === 'select'
                ? false
          ...
          >
            ...
            >
              <DatePicker
                ...
                mask="9999-99-99"
                @update:model-value="
                  filterModel.value = $event ? moment($event as Date).format('YYYY-MM-DD') : null
                "
              />
              <template v-else-if="column.type === 'select'">
                <template
                  v-if="!isBoolean(column.showFilterAddButton) || column.isMultiSelect === true"
                >
                  <MultiSelectColumnFilter
                    :label="
                      column.selectFilterHeader !== undefined
                        ? column.selectFilterHeader
                        : column.header
                    "
                    :options="
                      Array.isArray(column.selectOptions)
                        ? column.selectOptions
                        : (column.selectOptions?.list ?? [])
                    "
                    :slot-props="{ filterModel, filterCallback }"
          ...
          <template #body="{ data }">
            ...
            >
              <div
                :class="
                  column.bodyClassFunction
                    ? column.bodyClassFunction(lodashGet(data, column.name))
                    : ''
                "
              >
                <template v-if="column.html">
