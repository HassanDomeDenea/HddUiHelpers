    ...
    <div :data-sort-id="filter.id">
        ...
        >
          {{ t(operator) }}
            ...
            <span class="ms-2 whitespace-pre-wrap font-bold">{{ label }}</span>
            ...
            <template v-if="column.type ==='color' && !isNull(formattedValue)">
                <span
                    ...
                    :class="['!size-5']"
                    :style="{ backgroundColor: formattedValue }"></span>
            </template>
            <template v-else>
        ...
        <Popover ref="menuPopoverRef" @hide="onPopoverHide">
            <div class="flex min-w-[200px] flex-col items-stretch gap-1">
                ...
                ></Select>
                <FilterControl
                    v-model:filter-model="localFilterModel" :filter-callback="filterCallback"
                    :column="column" size="small" />
                <div v-if="getColumnCanShowFilterApplyButton(column)" class="mt-4 flex justify-end">
                    <Button
                        size="small" :label="t('Apply')" icon="i-mdi-check" severity="success"
