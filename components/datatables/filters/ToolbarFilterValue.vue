...
<template>
  <div :data-sort-id="filter.id">
    <ContextMenu v-if="!isPrinting" ref="filterDivContextMenuRef" :model="contextMenuItems" />
    <div
      ref="filterDivRef"
    ...
    >
      ...
      <template v-else>
        <span class="ms- 2 font-bold" v-html="isNull(formattedValue) ? t('?') : formattedValue" />
      </template>
    </div>
    <Popover ref="menuPopoverRef" @hide="onPopoverHide">
      <div class="flex min-w-[200px] flex-col items-stretch gap-1">
        ...
        />
        <FilterControl
          v-model:filter-model="localFilterModel"
