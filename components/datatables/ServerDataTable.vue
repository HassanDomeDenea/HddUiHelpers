...
import type PrintPaperForServerDataTable from 'HddUiHelpers/components/datatables/PrintPaperForServerDataTable.vue';
import {
  appendToUrl,
...
import { computed, nextTick, onActivated, onMounted, type Ref, ref, toValue, useTemplateRef, watch } from 'vue';
      ...
      if (column.selectOptionsKeyed) {
        ...
        if (!Array.isArray(column.selectOptions)) {
          ...
          obj[name] = column.selectOptions.reduce((carry: { [k2 in string]: string }, currentValue) => {
            carry[currentValue[(column.selectValueProperty ?? 'id') as 'id'] as string] = currentValue[
              (column.selectLabelProperty ?? 'name') as 'name'
            ] as string;
            return carry;
          }, {});
        }
...
function getColumnBody(rowData: T, column: ServerDataTableColumn<ServerDataTableColumnType, T>): string | string[] {
  ...
  if (column.type === 'price') {
    return formatters.formatPrice(
      value,
      typeof column.currency === 'string' ? column.currency : column.currency ? (rowData as T & { currency: string }) : undefined,
    );
  }
...
function clearFilterFor(columnName: string | ServerDataTableColumn): void {
  ...
  if (column) {
    filters.value[(column.filterField ?? column.fullFieldName) as string] = createFilterForColumn(column);
  }
}
...
function onFormSubmitted(item: T | T[], type: 'create' | 'update' | 'delete') {
  if (type === 'create') {
    ...
    emits('rowDeleted', item instanceof Array ? (item.map((i) => i[primaryKey]) as (string | number)[]) : (item[primaryKey] as string | number));
  }
  ...
  if (column.auditHistory) {
    ...
    if (url) {
      auditsPopoverRef.value?.showAudits(
        ...
        (_value) => (typeof column.formatter === 'function' ? column.formatter(_value, row, column.fullFieldName) : _value),
  ...
  >
    <ServerFormDialog ref="ServerFormDialogRef" v-bind="ServerFormDialogOptions">
      ...
      <template #beforeCancelButton="slotProps">
        <slot name="beforeCancelButton" v-bind="slotProps" />
      </template>
      <template #afterCancel="slotProps">
        <slot name="afterCancel" v-bind="slotProps" />
      </template>
      <template #beforeSubmitButton="slotProps">
        <slot name="beforeSubmitButton" v-bind="slotProps" />
      </template>
      <template #afterSubmitButton="slotProps">
        <slot name="afterSubmitButton" v-bind="slotProps" />
      </template>
      <template #beforeControls="slotProps">
        <slot name="beforeFormControls" v-bind="slotProps" />
      </template>
    </ServerFormDialog>
    ...
    >
      ...
      <template #printPageFooter="slotProps">
        <slot name="printPageFooter" :records="slotProps.records" :extra="slotProps.extra">
          <slot name="footer" :records="slotProps.records" :extra="slotProps.extra" />
        </slot>
      </template>
    </PrintPaperForServerDataTable>
    <ContextMenu
    ...
    >
      <template v-if="$slots.header" #header>
        <slot name="header" :records="records" :extra="extraData" />
      </template>
      <template v-if="$slots.footer" #footer>
        <slot name="footer" :records="records" :extra="extraData" />
      </template>
          ...
          >
            ...
            <template v-if="showOnlySortedIcon" #sorticon="slotProps">
              <template v-if="slotProps.sorted">
                ...
                />
                <i v-else :title="t('Descending')" class="i-mdi:sort-descending scale-y-[-1]" />
              </template>
            </template>
            <template #body="{ data }">
              <slot
              ...
              >
                <CellContent
                  ...
                  :row="data"
                  :size="computedSize"
                  :class="{ 'cell-has-edit-history': column.auditHistory }"
                  @contextmenu="onCellContextMenu($event, column, data)"
                />
              </slot>
            </template>
            <template
    ...
    <DataView v-if="withDataView" :value="records">
      <template #list="slotProps">
        <slot name="dataViewBody" :items="slotProps.items" :records="records" />
      </template>
    </DataView>
  </div>
</template>
...
.editing-cursor {
  cursor:
    url("../../assets/icons/pencil-green-700.svg") 0 24,
    pointer;
}
...
.dark {
  .row-open-cursor {
    cursor:
      url("../../assets/icons/cursor-click-blue-300.svg") 10 10,
      pointer;
  }
}

.cell-has-edit-history {
  @apply light:outline-purple-700/40 after:i-material-symbols:history relative rounded pe-1 outline-offset-4 after:absolute after:-end-6 after:top-1/2 after:-translate-y-1/2 after:text-gray-500 hover:outline-1 hover:after:content-[''] dark:outline-purple-300/40;
}
</style>
