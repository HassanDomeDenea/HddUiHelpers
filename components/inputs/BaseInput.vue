...
<template>
  <div class="form-control-wrapper" :class="{ 'flex gap-2 items-center ': inline, '!items-start': inline && ($slots.helper || error) }">
    <template v-for="i in [0, 1, 2]" :key="i">
      ...
      > <span :class="[icon, iconClass]" />
        <slot name="labelText">
          ...
          <template v-if="label">
            ...
            <Message v-if="required" variant="simple" size="small" :severity="error ? 'error' : 'contrast'" aria-hidden="true"><strong>*</strong></Message>
      ...
      <div v-if="(!controlBeforeLabel && i === 2) || (controlBeforeLabel && i === 0)" :class="[{ 'w-full': !controlBeforeLabel }, controlWrapperClass]">
        ...
        <Message v-if="error && typeof error === 'string'" :id="inputId ? `${inputId}-error` : ''" icon="i-heroicons-exclamation-triangle-20-solid" size="small" variant="simple" severity="error" class="mt-1 px-2">
          <span>{{ error }}</span>
        </Message>
        <Message v-if="slots.helper?.length" :id="inputId ? `${inputId}-desc` : ''" ref="helperSlotRef" size="small" variant="simple" severity="secondary" class="mt-1 px-2">
          <slot name="helper" />
        </Message>
      </div>
    </template>
