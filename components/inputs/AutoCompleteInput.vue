...
async function search(event: { query: string, offset?: number, limit?: number, onlyId?: boolean }) {
  ...
  let request
  if (props.url) {
    request = apiAxiosInstance.get(props.url, { params })
  }
  else {
    request = new Promise((resolve, reject) => {
      const result = props.options.filter((item) => {
        return item[props.optionLabelProperty].includes(params.name) && !(props.hideCurrentOption && item[props.optionLabelProperty].trim() === params.name)
      })
      resolve({
        data: {
          data: {
...
watch(() => dynamicValue.value, (value) => {
  if (props.noManualInput) {
...
function onAutocompleteInput(value) {
  if (props.noManualInput) {
    manualInput.value = value
  }
  else {
...
defineExpose({ focus, deselectAndMoveCaretToEnd, showList, hideList })
...
<template>
  <BaseInput v-bind="props">
    ...
    <template v-if="$slots.addon || clearable" #addon>
      <Button
        v-if="clearable && dynamicValue" :disabled="disabled" size="small" severity="danger" icon="i-mdi-times"
        @click="clear"
      />
      <slot name="addon" />
    </template>
