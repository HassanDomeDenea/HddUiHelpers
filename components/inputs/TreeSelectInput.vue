...
const localValue = computed({
    get() {
        if (value.value) {
            if (Array.isArray(value.value)) {
                return reduce(value.value, (carry, _id) => {
                    carry[_id] = true;
                    return carry;
                }, {});
            } else {
    ...
    set(_value) {
        if (_value) {
            const ids = Object.keys(_value).map(resolveNodeFromNodeKey).map(e => e[props.optionValueProperty]);
...
function convertOptionToTreeNodeIfNeeded(_option: any): TreeNodeWithId {
    if (_option.key && _option.label) {
        ...
        let isDisabled = typeof props.optionDisabledProperty === 'function' ? props.optionDisabledProperty({option: _option, value: value.value}) : (_option[props.optionDisabledProperty] ?? false);
...
function setVisibleElementValue(value: any) {
    if (value) {
        if (props.selectionMode === 'single') {
            ...
            inputRef.value.d_value = reduce(Array.isArray(value) ? value : [value], (carry, _id) => {
                carry[_id] = true;
                return carry;
            }, {});
        }
    ...
    <BaseInput ref="baseInputRef" v-bind="props" @click="focus">
        ...
        >
            ...
            <template v-if="$slots.option" #option="{ node, selected,expanded }">
                <slot
                    name="option" :node="node" :selected="selected"
                    :expanded="expanded">
