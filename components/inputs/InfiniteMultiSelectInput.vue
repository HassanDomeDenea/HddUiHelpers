...
watch(
    ...
    (_selectedItemIds: (string | number)[] | null) => {
        ...
        if (selectedItems.value?.length === _selectedItemIds.length && selectedItems.value?.every((_item, _itemIndex) => _item[props.optionValueProperty] === _selectedItemIds[_itemIndex])) {
            return;
        }
