...
async function submit(): Promise<void> {
  ...
  if (props.hasMultiRecords) {
    ...
    for (const fieldName in dirtyFields.value) {
      if (dirtyFields.value[fieldName]) {
        const modifiedValue = lodashGet(form.value, fieldName)
  ...
  <div>
        ...
        <template v-else-if="hasMultiRecords && differentValuesFields.includes(field.name)">
          ...
          >
            <Message severity="secondary" variant="simple" size="small">
              {{ t('Different Values') }}
              <Button
                v-tooltip="$t('Edit All')" size="small" variant="text" raised icon="pi pi-pencil"
                severity="warn" @click="restoreFieldDefault(field)"
              />
            </Message>
          </BaseInput>
        </template>
    ...
    <div class="mt-4 flex justify-between">
      ...
      <div class="focusableButtons">
        <slot
          name="submitButton" :in-edit-mode="inEditMode" :submit-button-ref="submitButtonRef"
