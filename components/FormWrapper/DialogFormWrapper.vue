...
function onFormSubmitted(type: 'create' | 'update', item: any) {
  toast.add({
    ...
    detail: type === 'create' ? t('Record Created Successfully!') : t('Record Updated Successfully!'),
    life: 3000,
  })
  if (type === 'create') {
    emits('created', item)
...
defineExpose({ edit, create, deleteItem, form })
</script>

<template>
  <Dialog
    v-model:visible="dialogVisible" modal :close-on-escape="!isDeleting" :closable="!isDeleting" :dismissable-mask="!isDeleting" keep-in-viewport
  ...
  >
    ...
    <FormWrapper ref="formRef" v-model="form" v-bind="formWrapperOptions" @reset="onResetButtonClicked" />
  </Dialog>
  <Dialog
    v-model:visible="deleteDialogVisible" modal close-on-escape closable dismissable-mask keep-in-viewport content-class="min-w-360px"
  >
    <template #header>
    ...
    <template #footer>
      <Button severity="secondary" :label="t('Cancel')" outlined :disabled="isDeleting" @click="deleteDialogVisible = false" />
      <Button ref="deleteConfirmButtonRef" tabindex="0" severity="danger" :label="t('Delete')" :loading="isDeleting" @click="onDeleteConfirmation" />
    </template>
  </Dialog>
</template>
