...
const { width: menuBarWidth, height: menuBarHeight } = useElementSize(menuBarRef, {}, {
    box: 'border-box'
});
...
<template>
    <Menubar ref="menuBarRef" :model="filteredItems" :class="navbarClass" class="z-2">
        ...
        <template #item="{ item, props, hasSubmenu, root }">
            <component
                :is="item.to ? 'router-link' : 'a'" :to="item.to" v-bind="props.action" :class="item.class"
                :active-class="activeItemClass">
                <i :class="item.icon" />
                <span>{{ item.label }}</span>
                <Badge v-if="item.badge" :class="{ 'ms-auto': !root, 'ms-2': root }" :value="item.badge" />
                <span v-if="item.shortcut" class="surface-border border-round surface-100 border-1 me-auto p-1 text-xs">{{ item.shortcut
                    }}</span>
                <i
                    v-if="hasSubmenu"
