...
function ke(i, e = {}) {
  ...
  const C = m(() => {
    if (e.childrenProperty) {
      const t = [], r = (n, s = []) => (s.push(B(n, e.childrenProperty)), n[e.childrenProperty]?.map((o) => r(o, s)), s);
      return f.value.map((n) => r(n, t)), t;
    } else
      return f.value;
  }), S = function(t) {
    if (!t)
    ...
    if (typeof t == "string" || typeof t == "number" ? n = S(t) : n = t, !n)
      return [];
    if (e.childrenProperty) {
      const s = [], o = (u, _ = []) => (_.push(B(u, e.childrenProperty)), u[e.childrenProperty]?.map((ie) => o(ie, _)), _);
      return r ? [n].map((u) => o(u, s)) : n[e.childrenProperty]?.map((u) => o(u, s)), s;
    } else
      return r ? [n] : [];
  }, re = function(t, r) {
    const n = O(t, r);
