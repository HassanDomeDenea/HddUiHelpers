...
export function createListStore<TItem = any>(
  ...
  const getNestedItem = (id: string | number) => {
    if (!id) {
      return undefined;
    }
  ...
  const getFlatChildrenOfItem = (id: TItem | string | number, includeSelf: boolean = false): TItem[] => {
    if (!id) {
      return [];
    }
    let _item: TItem;
  ...
  const getFlatIdsOfItem = (id: TItem | string | number, includeSelf?: boolean): (string | number)[] => {
