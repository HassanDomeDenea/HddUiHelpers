...
exports.mixed = {
  ...
  notType: function (_a) {
    ...
    var castMsg =
      originalValue != null && originalValue !== value ? ' (cast from the value `'.concat((0, yup_1.printValue)(originalValue, true), '`).') : '.';
    return type !== 'mixed'
  ...
  notType: function (params) {
    ...
    if (Array.isArray(value)) {
      if (value.length < typeLen) {
        return ''
          .concat(path, ' tuple value has too few items, expected a length of ')
          .concat(typeLen, ' but got ')
          .concat(value.length, ' for value: `')
          .concat((0, yup_1.printValue)(value, true), '`');
      }
      if (value.length > typeLen) {
        return ''
          .concat(path, ' tuple value has too many items, expected a length of ')
          .concat(typeLen, ' but got ')
          .concat(value.length, ' for value: `')
          .concat((0, yup_1.printValue)(value, true), '`');
      }
